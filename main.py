...
# ──────────────────────────────────────────────────────────────
# EVO EMOTE IDs – add this right after the other global lists
# ──────────────────────────────────────────────────────────────
EVO_EMOTES_ID = [
    909000063, 909000068, 909000075, 909000081, 909000085,
    909000090, 909000098, 909033001, 909033002, 909035007,
    909035012, 909037011, 909038010, 909038012, 909039011,
    909040010, 909041005, 909042008, 909045001, 909049010, 909051003
]
...
def send_likes(uid):
    try:
        # Updated API URL based on your information
        likes_api_response = requests.get(
            f"https://60-requestlipu.vercel.app/like?uid={uid}&server_name=ind",
            timeout=20
        )
        ...
        if likes_api_response.status_code == 200:
            ...
            # Extract relevant fields from the new API response structure
            status_code = api_json_response.get('status', 0)
            ...
            remaining_requests = api_json_response.get('Remaining_requests', 'N/A')
            player_level = api_json_response.get('Level', 'N/A')
            region = api_json_response.get('Region', 'N/A')
            ...
            if status_code == 1:
                ...
                if likes_added > 0:
                    return {
                        "status": "ok",
                        "message": (
                            f"[C][B][00FFFF]‎═══⚡ Likes Status ⚡═══\n"
                            f"[00FF00]Like Send successful!\n"
                            f"[FFFFFF]Player Name: [00FFFF]{player_name}  \n"
                            f"[FFFFFF]Player Level: [00FFFF]{player_level}  \n"
                            f"[FFFFFF]Like Given By Bot: [FF00FF]{likes_added}  \n"
                            f"[FFFFFF]Before: [00FFFF]{likes_before}  →[FFFFFF]After: [00FFFF]{likes_after}  \n"
                            f"[C][B][00FFFF]‎══════════════════\n"
                            f"[C][B][FFD700]Developer ATX ABIR❖"
                        )
                    }
                else:
                    return {
                        "status": "failed",
                        "message": (
                            f"[C][B][FFD700]═══⚠️ No Likes Added ⚠️═══\n"
                            f"[FFFFFF]Player Name: [00FFFF]{player_name}\n"
                            f"[FFD700]No likes were added in this request.\n"
                            f"[FFD700]══════════════════\n"
                            f"[C][B][00FFFF]Developer ATX ABIR❖"
                        )
                    }
            elif status_code == 2:
                # ⚠️ Max likes reached
                return {
                    "status": "failed",
                    "message": (
                        f"[C][B][FFD700]═══⚠️ Max Likes Reached ⚠️═══\n"
                        f"[FFFFFF]Player Name: [00FFFF]{player_name}\n"
                        f"[FFD700]⚠️ You've reached the daily maximum like limit!\n"
                        f"[FFD700]══════════════════\n"
                        f"[C][B][00FFFF]Developer ATX ABIR❖"
                    )
                }
            else:
                # Other status codes or unknown status
                return {
                    "status": "failed",
                    "message": (
                        f"[C][B][FF0000]═══❌ Unknown Status ❌═══\n"
                        f"[FFFFFF]API returned status: {status_code}\n"
                        f"[FFFFFF]Please try again later.\n"
                        f"[FF0000]═══════════════"
                    )
                }
            ...
            return {
                "status": "failed",
                "message": (
                    f"[C][B][FF0000]═══❌ API Error ❌═══\n"
                    f"[FFFFFF]The Like API returned an error!\n"
                    f"[FFFFFF]Status Code: {likes_api_response.status_code}\n"
                    f"[FFFFFF]Please try again later.\n"
                    f"[FF0000]═══════════════"
                )
            }
        ...
        return {
            "status": "failed",
            "message": (
                f"[C][B][FF0000]═══❌ Connection Failed ❌═══\n"
                f"[FFFFFF]Could not connect to the likes API server.\n"
                f"[FFFFFF]Please ensure the API server is running.\n"
                f"[FF0000]═══════════════"
            )
        }
    except Exception as e:
        # Handle any other unexpected errors
        logging.error(f"An unexpected error occurred in send_likes: {str(e)}")
        return {
            "status": "failed",
            "message": (
                f"[C][B][FF0000]═══❌ System Error ❌═══\n"
                f"[FFFFFF]An unexpected error occurred.\n"
                f"[FFFFFF]Check the logs for more details.\n"
                f"[FF0000]═══════════════"
            )
...
def get_random_avatar():
  avatar_list = [
        '902037031', '902049014', '902045006', '902042012', '902000363', 
        '902000191'
    ]
...
[FFD700]👑 NAME : [FFFF00] ATX ABIR 
[FFD700]📌 TELEGRAM : [00BFFF]@atxabir 
[FF1493]╚══════════════════════════╝""", idinv
                            )
                    )
                    senthi = False
...
[FFD700][b][c]Developer ATX ABIR❖ [/b]
[FF0000][c]━━━━━━━━━━━━━[/c]""",uid
                                )
                            )
                    else:
...
[FF0000]╠══════════╣
[FFD700]⚡ OwNer: [FFFFFF]ATX ABIR❖  
[FFD700]⚡ TELEGRAM : [FFFFFF]@atxabir 
[FFD700]Name on Telegram : [FFFFFF]ATX ABIR❖❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer ABIR❖⚡""", uid
                            )
                        )
                    except Exception as e:
                ...
                # ──────────────────────────────────────────────────────
                # COMMAND: /x <player_id>  → 60-second spam (invite + leave)
                # ──────────────────────────────────────────────────────
                if "1200" in data.hex()[0:4] and b"/x" in data:
                    try:
                        # ---------- 1. Extract UID ----------
                        cmd = re.split("/x ", str(data), flags=re.IGNORECASE)
                        if len(cmd) <= 1:
                            json_result = get_available_room(data.hex()[10:])
                            parsed = json.loads(json_result)
                            uid = parsed["5"]["data"]["1"]["data"]
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Usage: /x <player_id>\n"
                                "[FFFFFF]Example: /x 123456789", uid))
                            continue

                        player_id = cmd[1].split('(')[0].strip()
                        if "***" in player_id:
                        ...
                        # ---------- 3. Starting Message ----------
                        clients.send(self.GenResponsMsg(
                            f"[C][B][FFD700]Starting 60-second spam on [FFFF00]{fix_num(player_id)}", uid))

                        # ---------- 4. One-cycle helper ----------
                        def one_cycle():
                            try:
                                socket_client.send(self.skwad_maker())
                                time.sleep(0.02)                 # create squad
                                socket_client.send(self.changes(5))  # 6-player squad
                                time.sleep(0.20)
                                socket_client.send(self.invite_skwad(player_id))
                                time.sleep(0.01)
                                socket_client.send(self.leave_s())
                                time.sleep(0.12)
                                socket_client.send(self.changes(1))  # back to solo
                            except Exception as e_cycle:
                                logging.error(f"Cycle error: {e_cycle}")
                        ...
                        while time.time() - start_time < 60:
                            one_cycle()
                            cycle_count += 1
                            time.sleep(0.08)   # tiny pause between cycles

                        # ---------- 6. Completion Message ----------
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]60-second spam completed!\n"
                            f"[FFFFFF]Sent ~{cycle_count} invites to [FFFF00]{fix_num(player_id)}", uid))

                    except Exception as e:
                        logging.error(f"/x command crash: {e}")
                        try:
                            json_result = get_available_room(data.hex()[10:])
                            parsed = json.loads(json_result)
                            uid = parsed["5"]["data"]["1"]["data"]
                            clients.send(self.GenResponsMsg(
                                f"[C][B][FF0000]Error in /x: {str(e)}", uid))
                        except:
                            pass
                        restart_program()
...
[FFD700]✨ Developer ATX ABIR❖⚡""",
                ...
                if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00]SK SABIR  HERE [ffffff]BOT" in data:
                    pass
                else:
...
#393393993939	                    
                    if "1200" in data.hex()[0:4] and b"/like" in data:
                        try:
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    f"{generate_random_color()}⏳ ok sir, please wait...", uid
                                )
                            )
                            ...
                            logging.error(f"Error processing /like command: {e}. Restarting.")
                            restart_program()
...
╰━━╯""", uid
                            ...
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFD700]═══⚡ ATX ABIR BOT | HELP 1/5 ⚡═══
[FFFFFF]❤️ Change Team Group:
[00FFFF]/🗿3, /🗿4, /🗿5, /🗿6

[FFFFFF]Leave Squad & Go Solo:
[00FFFF]/🗿solo

[FFFFFF] Room Spam Requests:
[00FFFF]/room [uid]

[FFFFFF]Spam Friend Requests:
[FF0000]/🙃spam [uid]

[FFA500]/🙃inv [uid] [5]-> [FFFFFF]Invite Any Player""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFD700]═══⚡ ATX ABIR BOT | HELP 2/5 ⚡═══
[FFFFFF] Join Team Bot:
[00FFFF]/join [TeamCode]]

[FF00FF]⚡ Enjoyl The Emote Features !

[FFFFFF]Evo Emote command:
[FF0000]/🙃evo [uid]

[FFFFFF]Emote command:
[FF0000]/🤪play [uid] [emote_id]

[FFFFFF]Emote Fast repeat:
[FF0000]/💀fast [uid] [emote_id]
""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFD700]═══⚡ ATX ABIR BOT | HELP 3/5 ⚡═══
[FFFFFF]Lag Any Team:
[FF0000]/🙃lag [teamcode]
...
[FF0000]/🙃attack [teamcode]

[FFFFFF]Force Start a Team:
[FF0000]/🙃start [teamcode]
...
""", uid
                            ...
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFD700]═══⚡ ATX ABIR BOT | HELP 4/5 ⚡═══
[FFFFFF]Get 100 Likes:
[00FF00]/🙃like [uid]

[FFFFFF]Player Full Info:
[00FF00]/🙃info [uid]

[FFFFFF]Check Player Status:
[00FF00]/🙃status [uid]

[FFFFFF]Increase Visitors:
[00FF00]/🙃visit [uid]

[FFFFFF]Check Ban Status:
[00FF00]/🙃check [uid]""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
                                    self.GenResponsMsg(
                                        f"""[C][B][FFD700]═══⚡ ATX ABIR BOT | HELP 5/5 ⚡═══
[FFFFFF]Spam Join Requests:
[FF0000]/🙃sm [uid]

[FFFFFF]Ask Bharat AI:
[00FF00]/🙃ai [text]
...
# Command changed to /join tc, joins only once, does not leave
                ...
                if '1200' in data.hex()[0:4] and b'/lag' in data:
                    try:
                        ...
                        # ---- START MESSAGE ----
                        clients.send(self.GenResponsMsg(
                            f"[C][B][32CD32]Starting fast lag on {fix_num(room_id)}..", uid
                        ))
                        ...
                        # ---- FINAL MESSAGE ----
                        final_msg = (
                            f"[FFFFFF]Lag completed\n"
                            f"[FFFF00]team_code : {fix_num(room_id)}"
                        )
                        clients.send(self.GenResponsMsg(final_msg, uid))

                    except Exception as e:
                        logging.error(f"Lag command error: {e}")
                        restart_program()
                ...
                # ──────────────────────────────────────────────────────
                # COMMAND: /play <target_id1> <target_id2> ... <emote_id>
                # Example: /play 1963016904 2209708396 12956180610 6460827674 909000063
                # ──────────────────────────────────────────────────────
                if "1200" in data.hex()[0:4] and b"/play" in data:
                    try:
                        ...
                        # Extract: /play <uid1> <uid2> ... <emote_id>
                        command_parts = data.split(b'/play')[1].split(b'(')[0].decode().strip().split()
                        if len(command_parts) < 2:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Usage: /play <uid> <emote_id>", uid_sender))
                            continue

                        emote_id_str = command_parts[-1]
                        target_ids = command_parts[:-1]

                        if not emote_id_str.isdigit():
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Last argument must be a valid emote ID!", uid_sender))
                            continue

                        emote_id = int(emote_id_str)
                        valid_targets = [uid for uid in target_ids if uid.isdigit()]
                        if not valid_targets:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]No valid target IDs found!", uid_sender))
                            continue

                        # Start message with formatted IDs
                        fixed_ids = " | ".join(fix_num(tid) for tid in valid_targets)
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]Sending emote [00FF00] →\n", uid_sender))

                        # Send emote to each target
                        ...
                        # Success
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]Emote sent to {len(valid_targets)} player(s)!", uid_sender))

                    except Exception as e:
                        logging.error(f"Error in /play command: {e}. Restarting bot.")
                        restart_program()                



                                # ──────────────────────────────────────────────────────
                # COMMAND: /fast <target_id1> [target_id2] ... <emote_id>
                # Fast spam mode (replaces old @b)
                # ──────────────────────────────────────────────────────
                if "1200" in data.hex()[0:4] and b"/fast" in data:
                    try:
                        json_result = get_available_room(data.hex()[10:])
                        parsed_data = json.loads(json_result)
                        uid_sender = parsed_data["5"]["data"]["1"]["data"]

                        # Extract: /fast <uid1> <uid2> ... <emote_id>
                        command_parts = data.split(b'/fast')[1].split(b'(')[0].decode().strip().split()
                        if len(command_parts) < 2:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Usage: /fast <uid> <emote_id>", uid_sender))
                            continue
                        ...
                        # Start message
                        clients.send(self.GenResponsMsg(
                            f"[C][B][FF0000]FAST ATTACK → Emote on {len(valid_targets)} player(s)!", uid_sender))

                        # Spam loop: 300 times, ultra-fast
                        for _ in range(600):
                            for target_id in valid_targets:
                                emote_packet = self.send_emote(target_id, emote_id)
                                socket_client.send(emote_packet)
                            time.sleep(0.04)  # 10ms delay = ~100 packets/sec

                        # Final reply
                        clients.send(self.GenResponsMsg(
                            "[C][B][00FF00]Fast attack finished!", uid_sender))

                    except Exception as e:
                        logging.error(f"Error in /fast command: {e}. Restarting bot.")
                        restart_program()                
                ...
                # ──────────────────────────────────────────────────────
                # NEW COMMAND: /evo <uid1> <uid2> ...  →  EVO emote sequence (MULTI-TARGET)
                # ──────────────────────────────────────────────────────
                if "1200" in data.hex()[0:4] and b"/evo" in data:
                    try:
                        ...
                        # Extract all IDs after /evo
                        command_parts = data.split(b'/evo')[1].split(b'(')[0].decode().strip().split()
                        if not command_parts:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Usage: /evo <uid1> <uid2> ...", uid_sender))
                            continue

                        # Filter only digit-only IDs
                        target_ids = [uid for uid in command_parts if uid.isdigit()]
                        if not target_ids:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]No valid Player IDs found!", uid_sender))
                            continue

                        # Show formatted list of targets
                        fixed_ids = " | ".join(fix_num(tid) for tid in target_ids)
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]Starting EVO emote sequence on:\n{fixed_ids}", uid_sender))

                        # === Send each EVO emote to ALL targets ===
                        for emote_id in EVO_EMOTES_ID:
                            for target_id in target_ids:
                                emote_packet = self.send_emote(target_id, emote_id)
                                socket_client.send(emote_packet)
                            time.sleep(6)  # 5-second delay after full round

                        # Final confirmation
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]EVO emote sequence completed for all {len(target_ids)} targets!", uid_sender))

                    except Exception as e:
                        logging.error(f"Error in /evo command: {e}. Restarting bot.")
                        restart_program()

                # /newlag <uid1> <uid2> ... → 1 Invite Each + 200x Fast Switch
                # Only 3 Messages: Start → Processing → End
                # ──────────────────────────────────────────────────────
                if "1200" in data.hex()[0:4] and b"/newlag" in data:
                    try:
                        json_result = get_available_room(data.hex()[10:])
                        parsed_data = json.loads(json_result)
                        uid_sender = parsed_data["5"]["data"]["1"]["data"]
                        ...
                        # Extract UIDs
                        command_parts = data.split(b'/newlag')[1].split(b'(')[0].decode().strip().split()
                        if not command_parts:
                            clients.send(self.GenResponsMsg(
                                "[C][B][FF0000]Usage: /newlag <uid1> <uid2>...", uid_sender))
                            continue

                        target_ids = [uid for uid in command_parts if uid.isdigit()]
                        if not target_ids:
                            clients.send(self.GenResponsMsg("[C][B][FF0000]No valid UIDs!", uid_sender))
                            continue

                        fixed_targets = " | ".join(fix_num(tid) for tid in target_ids)
                        total = len(target_ids)

                        # === 1. STARTING MESSAGE ===
                        clients.send(self.GenResponsMsg(
                            f"[C][B][FF0000]NEW LAG ATTACK STARTED!\n"
                            f"[FFFFFF]Targets: [FFFF00]{fixed_targets}\n"
                            f"[FFD700]{total} Players → 1 Invite + 200x Switch", uid_sender))

                        # === 2. PROCESSING MESSAGE (Only Once) ===
                        clients.send(self.GenResponsMsg(
                            f"[C][B][FFA500]Starting + 200x Fast Group Switch...", uid_sender))

                        # Create squad
                        socket_client.send(self.skwad_maker())
                        time.sleep(1.0)

                        # Send 1 invite to each
                        for target_id in target_ids:
                            socket_client.send(self.invite_skwad(target_id))
                            time.sleep(0.3)

                        # 200x Fast Switch (5→4→5→4...)
                        for i in range(150):
                            squad_size = 5 if (i % 2 == 0) else 6
                            socket_client.send(self.changes(squad_size - 1))
                            time.sleep(0.33)
                        ...
                        # === 3. ENDING MESSAGE ===
                        clients.send(self.GenResponsMsg(
                            f"[C][B][00FF00]Fast Group Switch complete\n"
                            f"[FFFFFF]Target: [FFFF00]{total} Players\n"
                            f"[00FF00]200x Switch Done", uid_sender))

                    except Exception as e:
                        logging.error(f"Error in /newlag: {e}")
                        clients.send(self.GenResponsMsg(
                            f"[C][B][FF0000]Error: {str(e)}", uid_sender))
                        restart_program()                                                            
